...
public class ArrowWrite {
    ...
    public void setupWrite(String filename, boolean useCustom) throws Exception {
        if (this.outputStream != null) {
            throw new IOException("output stream is not null!");
        }

        File arrowFile = new File(filename);
        this.outputStream = new FileOutputStream(arrowFile);
        ...
        if (!useCustom) {
            ...
            this.arrowFileWriter = new ArrowFileWriter(root,
                    provider,
                    ((FileOutputStream)(this.outputStream)).getChannel());
        } else {
            /* custom channel implementation in ArrowOutputStream */
            this.arrowFileWriter = new ArrowFileWriter(root,
                    provider,
                    new ArrowOutputStream(this.outputStream));
        }
    ...
    public void setupWrite() throws Exception {
        ...
        System.out.println("Generated " + this.entries + " data entries , batch size " + batchSize + " useNullValues " + this.useNullValues);
    }
