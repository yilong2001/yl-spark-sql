...
statement
    ...
    | UPDATE target=tableIdentifier SET sets=colvaluelist fromClause
        WHERE where=booleanExpression                                  #updateSetFromCommand
    | TRAIN MODEL target=IDENTIFIER fromClause INTO path=STRING        #trainModelFromCommand
    | REGISTER MODEL target=IDENTIFIER FROM path=STRING AS name=IDENTIFIER #registerModeFunctionCommand
    ;
