...
case class HBaseRelation(parts: Array[Partition],
                         hbaseOption: HBaseOption)(@transient val sparkSession: SparkSession)
  ...
  // TODO: Check if HBASERDD.compileFilter can accept input filters
  ...
  override def buildScan(requiredColumns: Array[String], filters: Array[Filter]): RDD[Row] = {
    ...
    HBaseRDD.scanTable(
      ...
      filters,
      parts,
      hbaseOption).asInstanceOf[RDD[Row]]
  }

  override def toString: String = {
    val partitioningInfo = if (parts.nonEmpty) s" [numPartitions=${parts.length}]" else ""
    // credentials should not be included in the plan output, table information is sufficient.
    s"HBaseRelation(${hbaseOption.hTableName})" + partitioningInfo
  }

  override def schema: StructType = null//AvroSchemaConverter.transferAvroSchema(new Schema.Parser().parse(hbaseOptions.tableAvsc)).asInstanceOf[StructType]
